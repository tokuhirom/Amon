...
=head2 HTTP::MobileAgent との連携
    ...
    use base qw/Amon::Request/;
    use HTTP::MobileAgent;
    sub mobile_agent { $_[0]->{mobile_agent} ||= HTTP::MobileAgent->new($_[0]->headers) }
    1;

として、カスタムリクエストクラスを作成します。こうすることにより、
HTTP::MobileAgent を気軽につかえるようになります。

次に、これを利用するために、以下のように記述します。
