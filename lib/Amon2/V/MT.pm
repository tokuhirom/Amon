...
sub import {
    ...
    if (@_>0 && shift eq '-base') {
        ...
        try {
            Amon2::Util::load_class($context_class);
        } catch {
...
sub {
    ...
    if (my \$parent = delete \$Amon2::V::MT::render_context->{extends}) {
        \$out = \$Amon2::V::MT::render_context->{c}->__load_internal(\$parent);
    }
    \$out;
}
...
...
Amon2::V::MT - Amon2 Text::MicroTemplate View Class
...
This is Text::MicroTemplate wrapper for Amon2.
    ...
    MyApp->bootstrap(
        {
            config => {
                'V::MT' => {
                    cache_mode => Amon2::V::MT::CACHE_FILE(),
                }
            }
        }
    );
