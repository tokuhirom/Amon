...
sub run_jquery {
    ...
    print {$fh} $xslate->render_string(<<'...', +{ file => $0, basename => basename($url), files => Data::Dumper::Dumper({ "static/js/" . basename($url) => $jquery})});
...
sub run_bootstrap {
    ...
    for my $url (qw(
        ...
        $files->{"static/bootstrap/$basename"} = fetch($url);
    }
